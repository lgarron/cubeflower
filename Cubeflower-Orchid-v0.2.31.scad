...
function unengraved(s) =
  struct_set(
    s,
    [
      "INCLUDE_VERSION_ENGRAVING",
      false, //
    ],
    grow=false
  );
...
default_variant = struct_set(
  [], [
    "INCLUDE_INNER_STAND_ENGRAVING",
    false, //
    "INCLUDE_SIDE_ENGRAVING",
    false, //
    "FILL_VERSION_ENGRAVING",
    false, //
    "INCLUDE_SHELL",
    true, //
    "INCLUDE_STAND",
    true, //
    "INCLUDE_SUPPORT_FOR_HINGE_CONNECTORS",
    false, //
  ]
);
fancy_shell_variant = struct_set(
  default_variant,
  [
    "INCLUDE_SIDE_ENGRAVING",
    true, //
...
fancy_stand_variant = struct_set(
  default_variant,
  [
    "INCLUDE_INNER_STAND_ENGRAVING",
    true, //
...
module lat(i, mirror_scale) {
  scale([mirror_scale, 1, 1]) difference() {
      translate(
        [
          -LARGE_VALUE / 2,
          i * LAT_WIDTH * 2 + LAT_WIDTH / 2 + mirror_scale * LAT_WIDTH / 2 - SLIDING_CLEARANCE,
          -BASE_HEIGHT - _EPSILON - LARGE_VALUE,
        ]
      )
        cube(
          [
            LARGE_VALUE,
            LAT_WIDTH + SLIDING_CLEARANCE * 2,
            GEAR_MAX_RADIAL_DIVERGENCE * 2 + _EPSILON + DEFAULT_CLEARANCE + LARGE_VALUE,
          ]
        );
...
module lats_bottom_slant_right() {
  ...
  color("red") translate(
      [BASE_LATTICE_OFFSET_OUTER - 2 * DEFAULT_CLEARANCE, 0, -HINGE_THICKNESS + __SMALL_HINGE__HINGE_SHAVE]
    ) union() {
...
module bottom_rounding_negative() {
  render() difference() {
      translate(
        [
          LARGE_VALUE / 2 + OUTER_SHELL_INNER_WIDTH / 2 - BOTTOM_ROUNDING_RADIUS_X,
          0,
          -LARGE_VALUE / 2 - BASE_HEIGHT + BOTTOM_ROUNDING_RADIUS_Z,
        ]
      ) cube(LARGE_VALUE, center=true);
      minkowski() {
        translate(
  ...
  if (INCLUDE_STAND_PLUGS) {
    ...
    render() minkowski() {
        union() {
          translate([0, -15, -PLUG_HEIGHT])
            cylinder(
              h=PLUG_HEIGHT + INNER_STAND_CLEARANCE + _EPSILON - PLUG_TOP_EXPANSION_HEIGHT,
              r=PLUG_STEM_RADIUS
            );
          h = PLUG_TOP_EXPANSION_HEIGHT + HINGE_CONNECTOR_ALIGNMENT_EXTRA_HEIGHT + INNER_STAND_CLEARANCE + INNER_STAND_PLUG_HORIZONTAL_CLEARANCE + _EPSILON;
          translate([0, -15, -PLUG_TOP_EXPANSION_HEIGHT])
            cylinder(
              h=h, r1=PLUG_STEM_RADIUS,
    ...
    render() minkowski() {
        translate([0, -15, -PLUG_HEIGHT - negative_clearance])
          cylinder(
            h=PLUG_HEAD_HEIGHT - HINGE_PLUG_ROUNDING, // Note: the height purposely excludes clearance for a snug top.
            r=PLUG_HEAD_RADIUS + negative_clearance / 2 - HINGE_PLUG_ROUNDING // We purposely reduce the top clearance for a snug fit.
          );
  ...
  difference() {
    ...
    duplicate_and_mirror([0, 1, 0]) translate(
        [
          ...
          10 + __SMALL_HINGE__PLUG_VERTICAL_CLEARANCE - negative_horizontal_clearance,
          -HINGE_THICKNESS / 2,
        ]
      )
        cube(
          [
            HINGE_THICKNESS * 2 - 2 * __SMALL_HINGE__CONNECTOR_OUTSIDE_CLEARANCE + 2 * negative_horizontal_clearance,
            10 - 2 * __SMALL_HINGE__PLUG_VERTICAL_CLEARANCE + 2 * negative_horizontal_clearance,
            HINGE_THICKNESS / 2 + INNER_STAND_CLEARANCE + HINGE_CONNECTOR_ALIGNMENT_EXTRA_HEIGHT + negative_vertical_clearance,
          ]
        );
...
module pre_lats_right() {
  rotate_for_lid_right(LID_OVEROPENED_FLAT_ANGLE) difference() {
      rotate_for_lid_right(LID_OVEROPENED_FLAT_ANGLE) difference() {
          ...
          translate(
            [
              -LARGE_VALUE / 2 + BASE_LATTICE_OFFSET_INNER,
              0,
              -LARGE_VALUE / 2 - BASE_HEIGHT + GEAR_MAX_RADIAL_DIVERGENCE,
            ]
          ) cube(LARGE_VALUE, center=true);
        }
...
module hinge() {
  ...
  difference() {
        ...
        duplicate_and_mirror_with_corresponding_lats_and_bottom_rounding_difference() {
          union() {
            translate([BASE_LATTICE_OFFSET_INNER, -OUTER_SHELL_INNER_WIDTH / 2, -BASE_HEIGHT]) cube(
                [
                  OUTER_SHELL_INNER_WIDTH / 2 - BASE_LATTICE_OFFSET_INNER,
    ...
    duplicate_and_mirror([0, 1, 0]) translate(
        [
          -HINGE_THICKNESS + _EPSILON,
          15 - 5 + __SMALL_HINGE__PLUG_VERTICAL_CLEARANCE,
          -HINGE_THICKNESS + _EPSILON,
        ]
      )
        cube(
          [
            HINGE_THICKNESS * 2 - 2 * _EPSILON,
            10 - 2 * __SMALL_HINGE__PLUG_VERTICAL_CLEARANCE,
            HINGE_CONNECTOR_BOTTOM_CLEARANCE - _EPSILON,
          ]
        );
...
module version_text_engraving() {
  union() {
    rotate_for_lid_left(OPENING_ANGLE_EACH_SIDE) {
      translate(
        [
          -ENGRAVING_TEXT_CORNER_OFFSET_X,
          -OUTER_SHELL_OUTER_WIDTH / 2,
          ENGRAVING_TEXT_CORNER_OFFSET_Y - BASE_HEIGHT,
        ]
...
assert(
  SNAP_CONNECTOR_TOP_ANGLE <= 45
); // The code below assumes this, in order to avoid creating shapes unbounded in size.
assert(
  SNAP_CONNECTOR_SIDE_ANGLE <= 45
); // The code below assumes this, in order to avoid creating shapes unbounded in size.
;
...
module snap_connector(angle) {
  ROUNDING_Y = 1 / cos(angle) * (
    -SNAP_CONNECTOR_RADIUS - 2 * SNAP_CONNECTOR_RADIUS * pow(cos(angle), 2) + SNAP_CONNECTOR_RADIUS * sin(angle) - 2 * SNAP_CONNECTOR_RADIUS * pow(sin(angle), 2)
  );
          ...
          translate([0, -DEFAULT_CLEARANCE / 2 / cos(angle), 0]) union() {
              ...
              difference() {
                ...
                translate([SNAP_CONNECTOR_RADIUS, ROUNDING_Y, -_EPSILON]) rotate([0, 0, 90]) mirror([1, 0, 0])
                      cube(
                        [
                          2 * SNAP_CONNECTOR_RADIUS,
                          2 * SNAP_CONNECTOR_RADIUS,
                          OUTER_SHELL_THICKNESS + 2 * _EPSILON,
                        ]
                      );
              }
            }
...
module snap_connector_negative(angle) {
  render() translate([-DEFAULT_CLEARANCE, 0, 0]) difference() {
        minkowski() {
          ...
          cylinder(
            h=SNAP_CONNECTOR_NEGATIVE_CURVE_COMPENSATION_THICKNESS * 2, r=DEFAULT_CLEARANCE / 2,
            center=true
          );
        }
...
module side_engravings(epsilon = _EPSILON) {
  duplicate_and_rotate([0, 0, 180]) rotate_for_lid_right(OPENING_ANGLE_EACH_SIDE - LID_OVEROPENED_FLAT_ANGLE)
      translate(
        [
          OUTER_SHELL_INNER_WIDTH / 2 + OUTER_SHELL_THICKNESS,
          0,
          INNER_STAND_FLOOR_ELEVATION + CUBE_EDGE_LENGTH / 2 + SIDE_ENGRAVING_EXTRA_ELEVATION,
        ]
    ...
    difference() {
      ...
      union() {
        render() duplicate_and_mirror_with_corresponding_lats_and_bottom_rounding_difference() difference() {
                ...
                render() minkowski_shell() {
                    union() {
                      ...
                      translate([0, -OUTER_SHELL_INNER_WIDTH / 2, OUTER_SHELL_THICKNESS - BASE_HEIGHT])
                        cube(
                          [
                            OUTER_SHELL_INNER_WIDTH / 2,
                            OUTER_SHELL_INNER_WIDTH,
                            BASE_HEIGHT + INNER_STAND_FLOOR_ELEVATION,
                          ]
                        );
                    }
              ...
              translate(
                [
                  -BASE_LATTICE_OFFSET_OUTER,
                  -(OUTER_SHELL_INNER_WIDTH + 2 * OUTER_SHELL_THICKNESS) / 2,
                  -BASE_HEIGHT - _EPSILON,
                ]
              )
                cube(
                  [
                    BASE_LATTICE_OFFSET_OUTER * 2,
                    OUTER_SHELL_OUTER_WIDTH,
                    GEAR_MAX_RADIAL_DIVERGENCE + _EPSILON,
                  ]
                );
      ...
      translate(
        [
          0,
          0,
          LARGE_VALUE / 2 - BASE_LATTICE_COMPLEMENT_OFFSET - DEFAULT_CLEARANCE_FACTOR_FOR_LID_SHAVE_TO_MATCH_LATS_AT_90_DEGREES * DEFAULT_CLEARANCE,
        ]
